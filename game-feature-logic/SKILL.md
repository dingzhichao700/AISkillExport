...
- Treat an established workbook's font family and font size as workbook-wide rules when the workbook uses one consistent typography baseline. Before modifying or adding cells, read and reuse the actual font family and size from a representative existing cell; never substitute a visually similar font or guess the size. After export, compare the computed styles of at least one original cell and one changed cell to confirm they match, unless the workbook or the user explicitly requires a typography exception.
